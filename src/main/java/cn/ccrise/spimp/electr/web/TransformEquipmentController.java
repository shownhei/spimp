...
//		 * (4):拉紧装置-TensioningDevice;
	...
	public void exportExcel(HttpSession httpSession,HttpServletResponse response, Long deviceClass,
		...
		new ExcelHelper<AnnualOil>().genExcelWithTel(httpSession, response, "electr/equipment/transform-equipments.xls", root, "运输设备", new String[] {  "运输设备" }, new ExcelCallBackInteface() {
			@Override
			public void process(Workbook book, HashMap<String, Object> root) {
				...
				for(TransformEquipment equipment:result){
					drawTensioning(tensioningMap, maxRowMap,  mainRecordCols,
							reducerCols, electromotorCols, brakeCols,tensioningCols, sheet, result,equipment,mainStartRow);
					drawBrake( maxRowMap,  mainRecordCols,
							reducerCols, electromotorCols, brakeCols,tensioningCols, sheet, result,equipment,mainStartRow);
					drawElectromotor( maxRowMap,  mainRecordCols,
							reducerCols, electromotorCols, brakeCols,tensioningCols,  sheet, result,equipment,mainStartRow);
					drawReducers( maxRowMap,  mainRecordCols,
							reducerCols, electromotorCols, brakeCols,tensioningCols,  sheet, result,equipment,mainStartRow);
				...
				for(TransformEquipment equipment:result){
					...
					sheet.addMergedRegion(temp);
					temp = new CellRangeAddress(mainStartRow2, mainStartRow2 + rows - 1,
							colIndex, colIndex++);sheet.addMergedRegion(temp);
					temp = new CellRangeAddress(mainStartRow2, mainStartRow2 + rows - 1,
							colIndex, colIndex++);sheet.addMergedRegion(temp);
					temp = new CellRangeAddress(mainStartRow2, mainStartRow2 + rows - 1,
							colIndex, colIndex++);sheet.addMergedRegion(temp);
				   ...
				   int reducerStartCol=mainRecordCols;
					...
					if(maxRowMap.get(equipment.getId())>1&&(rowCount==0||rowCount==1)){
						...
						mergeCells(maxRowMap,  sheet, equipment,
								reducerStartRow,mainRecordCols,reducerCols);
					}
			}
			private void mergeCells(HashMap<Long, Integer> maxRowMap,Sheet sheet,
				...
				for(int i=0;i<cols;i++){
					CellRangeAddress temp = new CellRangeAddress(startRow, startRow + maxRowMap.get(equipment.getId()) - 1,
			...
			private void drawBrake(
				   ...
				   int brakeStartCol=mainRecordCols+reducerCols+electromotorCols;
					...
					if(maxRowMap.get(equipment.getId())>1&&(rowCount==0||rowCount==1)){
						reducerStartRow=mainStartRow;
						mergeCells(maxRowMap,  sheet, equipment,
								reducerStartRow,mainRecordCols+reducerCols+electromotorCols,brakeCols);
					}
			}
			/**
			 * 拉紧装置
			...
			private void drawTensioning(
				...
				int tensioningStartCol=mainRecordCols+reducerCols+electromotorCols+brakeCols;
					...
					if(maxRowMap.get(equipment.getId())>1&&(rowCount==0||rowCount==1)){
						...
						mergeCells(maxRowMap,  sheet, equipment,
								tensioningStartRow,tensioningStartCol,tensioningCols);
					}
			}
