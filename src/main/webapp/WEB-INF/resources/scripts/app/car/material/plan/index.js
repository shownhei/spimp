define(function(require, exports, module) {
	...
	var operateUri = '/car/material/plans';
	// 提示信息
	...
	$('#add_detail-save,#add_detail-saveandclose').click(function(event) {
		...
		object.plan = plan;
		// 处理属性
		$.post('/car/material/plan-details', JSON.stringify(object), function(data) {
			if (data.success) {
				var el = $(event.target);
	...
	$('#edit_detail-save').click(function(event) {
		...
		object.plan = plan;
		// 处理属性
		$.put('/car/material/plan-details/'+object.id, JSON.stringify(object), function(data) {
			if (data.success) {
	...
	$('#export_plan').click(function() {
		...
		window.location.href = '/car/material/plans/export-excel?' + Utils.form.buildParams('search-form');
	});
		...
		$(document).click(function(event) {
			...
			if (detailId) {
				if (el.attr('buttonType') === 'edit') {
					...
					$.get('/car/material/plan-details/' + selectId, function(data) {
					...
					// 删除明细
					$.del('/car/material/plan-details/' + detailId, function(data) {
						$('#submit').trigger('click');
						Utils.modal.hide('remove');
					});
				}
