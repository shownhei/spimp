define(function(require, exports, module) {
	...
	name : 'curPersonNum',
	render : function(value) {
	    return value;// '<a href="#" data-name=' + value + '>' + value +
	    // '</a>';
	}
    }, {
	header : '时间',
	 ...
	 * , { header : '轨迹', name : 'id', render : function(value) { return '<a
	 * href="#" link-name=' + value.staffId + '>' + '轨迹' + '</a>'; } }
	 */];
	...
	// 控制显示机构树
	groupTree = Utils.tree.group('groupTree', contextPath
		+ '/system/groups/2?label=mine', function(event, treeId,
		treeNode, clickFlag) {
	...
	$('.page-content .row-fluid .span9').removeClass('span9 hide')
		.addClass('span12');
    }
	...
	// 初始化grid的url
	gridUrl1 = contextPath
		+ '/location/location-areas?orderBy=id&order=desc&pageSize='
		+ pageSize;
	gridUrl2 = contextPath
		+ '/location/location-stations?orderBy=id&order=desc&pageSize='
		+ pageSize;
	gridUrl3 = contextPath
		+ '/location/location-staffs?orderBy=id&order=desc&pageSize='
		+ pageSize;
	...
	if (number !== undefined) { // 根据number来判断是点击机构树还是点击tab
	    ...
	    if (groupTree !== undefined
		    && groupTree.getSelectedNodes()[0] !== undefined) {
		gridUrl = gridUrl + '&mineId='
			+ groupTree.getSelectedNodes()[0].number;
	    }
	}
