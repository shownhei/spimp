		...
		$.get('history-curve?nodeIds=' + param.nodePlace + '&date=' + param.date, function(data) {
			...
			for (var j = 0; j < result.length; j++) {
				...
				for (var i = 0; i < dataLength; i++) {
					dateTimeArray.push(getTimes(currentNodeData[i].dataTime));
					avgArray.push(currentNodeData[i].avgData);
					maxArray.push(currentNodeData[i].maxData);
					minArray.push(currentNodeData[i].minData);
				}
			}
