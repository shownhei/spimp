define(function(require, exports, module) {
	...
	function validate(showType, model){
		...
		if (model.carCategory === '') {
			errorMsg.push('请输入车辆类型');
		}

		if (model.models === '') {
			errorMsg.push('请输入车辆型号');
		}
		...
		if (model.carStatus === '') {
			errorMsg.push('请输入车辆状态');
		}

		if (model.carStatus !== '' && !$.isNumeric(model.carStatus)) {
			errorMsg.push('车辆状态为数字格式');
		}

		if (model.engineSize !== '' && !$.isNumeric(model.engineSize)) {
			errorMsg.push('排气量为数字格式');
		}

		if (model.engineNumber !== '' && !$.isNumeric(model.engineNumber)) {
			errorMsg.push('发动机号为数字格式');
		}
