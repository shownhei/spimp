define(function(require, exports, module) {
	...
	function changeButtonsStatus(selected, data) {
		if (selected) {
			...
			if (data.account.id === window.parseInt(accountId)) {
				Utils.button.enable([ 'edit', 'remove', 'move' ]);
			} else {
				Utils.button.disable([ 'edit', 'remove', 'move' ]);
			}
		} else {
			Utils.button.disable([ 'edit', 'remove', 'move' ]);
		}
	...
	// 文件移动
	$('#move').click(function() {
		if (Utils.button.isDisable('move')) {
			return;
		}
		var tree = $.fn.zTree.getZTreeObj("move-tree");
		var nodes = tree.getNodes();
		tree.reAsyncChildNodes(nodes[0], "refresh");
		Utils.modal.show('move');
		$('#move-targert-folder').val("");
	});
	// 编辑
	$('#edit').click(function() {
	...
	// 移动确认 docId,Long folderId
	$('#move-save').click(function() {
		var folderId = $('#move-targert-folder').val();
		if (!folderId) {
			Utils.modal.showAlert('请选择目标文件夹', '提示', 'noFolderSelect');
			return;
		} else {
			var selectId = grid.selectedData('id');
			$.get('/spmi/document/move?folderId=' + folderId + '&docId=' + selectId, function(data) {
				grid.refresh();
				Utils.modal.hide('move');
			});
		}
	});
	// 搜索
	...
	moveNodeOperation.onClick = function(event, treeId, treeNode) {
		$.fn.zTree.getZTreeObj("move-tree").expandNode(treeNode, true);
		if (treeNode.parentId !== -1) {
			$('#move-targert-folder').val(treeNode.id);
		} else {
			utils.modal.showAlert("根目录不能存放文件", "警告", "docMoveAlert");
		}
	};
	var moveSetting = {
		view : {
		...
		callback : {
			...
			onClick : function(event, treeId, treeNode) {
				moveNodeOperation.onClick(event, treeId, treeNode);
			},
			onAsyncSuccess : function(event, treeId) {
				// 自动展开第一个节点
	...
	var moveTree = $.fn.zTree.init($("#move-tree"), moveSetting);
	// create-folder
	// 新建
