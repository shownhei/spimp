define(function(require, exports, module) {
	...
	// 配置表格
	var defaultUrl = contextPath + '/spmi/quality/electro-grades?'
	...
	$('#create-save').click(function() {
		...
		$.post(contextPath + '/spmi/quality/electro-grades', JSON.stringify(object), function(data) {
			if (data.success) {
				grid.refresh();
	...
	$('#edit').click(function() {
		...
		$.get(contextPath + '/spmi/quality/electro-grades/' + selectId, function(data) {
	...
	$('#edit-save').click(function() {
		...
		var selectId = grid.selectedData('id');
		object.id = selectId;
		$.put(contextPath + '/spmi/quality/electro-grades/' + selectId, JSON.stringify(object), function(data) {
			if (data.success) {
				grid.refresh();
	...
	$('#remove-save').click(function() {
		var selectId = grid.selectedData('id');
		$.del(contextPath + '/spmi/quality/electro-grades/' + selectId, function(data) {
			grid.refresh();
	...
	$('#calculate-query').click(
			function() {
				...
				var html = '';
				$.get(contextPath + '/spmi/quality/electro-grades?orderBy=year,month&order=desc,desc&pageSize=1000&year=' + object.year + '&month='
					...
					$.each(data.data.result, function(key, value) {
						...
						html += '<td align="center">' + value.antiexplosion + '</td>';
						html += '<td align="center">' + value.maintain + '</td>';
						html += '<td align="center">' + value.accident + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 0, 9) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 10, 23) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 24, 33) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 34, 45) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 46, 48) + '</td>';
						html += '<td align="center">' + value.score + '</td>';
						html += '</tr>';

						avgScores[0] += sum(value.gradeRecords, 0, 9);
						avgScores[1] += sum(value.gradeRecords, 10, 23);
						avgScores[2] += sum(value.gradeRecords, 24, 33);
						avgScores[3] += sum(value.gradeRecords, 34, 45);
						avgScores[4] += sum(value.gradeRecords, 46, 48);
