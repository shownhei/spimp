define(function(require, exports, module) {
	...
	// 根据页面高度自适应
	$('#create-grade-record-table,#edit-grade-record-table,#view-grade-record-table').height($(window).height() - 250);
	...
	// 配置表格
	var defaultUrl = contextPath + '/spmi/quality/tunnelling-grades?'
	...
	$('#create-save').click(function() {
		...
		$.post(contextPath + '/spmi/quality/tunnelling-grades', JSON.stringify(object), function(data) {
			if (data.success) {
				grid.refresh();
	...
	$('#edit').click(function() {
		...
		$.get(contextPath + '/spmi/quality/tunnelling-grades/' + selectId, function(data) {
	...
	$('#edit-save').click(function() {
		...
		var selectId = grid.selectedData('id');
		object.id = selectId;
		$.put(contextPath + '/spmi/quality/tunnelling-grades/' + selectId, JSON.stringify(object), function(data) {
			if (data.success) {
				grid.refresh();
	...
	$('#remove-save').click(function() {
		var selectId = grid.selectedData('id');
		$.del(contextPath + '/spmi/quality/tunnelling-grades/' + selectId, function(data) {
			grid.refresh();
	...
	$('#calculate-query').click(
			function() {
				...
				var html = '';
				$.get(contextPath + '/spmi/quality/tunnelling-grades?orderBy=year,month&order=desc,desc&pageSize=1000&year=' + object.year + '&month='
					...
					$.each(data.data.result, function(key, value) {
						...
						html += '<td align="center">' + sum(value.gradeRecords, 0, 2) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 3, 5) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 6, 9) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 10, 13) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 14, 14) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 15, 17) + '</td>';
						html += '<td align="center">' + sum(value.gradeRecords, 18, 21) + '</td>';
						html += '<td align="center">' + value.score + '</td>';
						html += '</tr>';

						avgScores[0] += sum(value.gradeRecords, 0, 2);
						avgScores[1] += sum(value.gradeRecords, 3, 5);
						avgScores[2] += sum(value.gradeRecords, 6, 9);
						avgScores[3] += sum(value.gradeRecords, 10, 13);
						avgScores[4] += sum(value.gradeRecords, 14, 14);
						avgScores[5] += sum(value.gradeRecords, 15, 17);
						avgScores[6] += sum(value.gradeRecords, 18, 21);
					...
					if (data.data.totalCount !== 0) {
						...
						html += '<td align="center">' + (avgScores[6] / data.data.totalCount).toFixed(1) + '</td>';
