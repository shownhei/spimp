	...
	private static Map<String, Object> prepareDataMap(final String entityName, final String packageName,
		...
		if(clazz != null) {
			...
			for (Field field : fields) {
				...
				PageFields column = (PageFields) field.getAnnotation(PageFields.class);
				...
				if(query){
					if("date".equalsIgnoreCase(columnType) && !dateQuery){
						...
						dbQuery.append(",Restrictions.ilike(\"").append(columnName).append("\", search, MatchMode.ANYWHERE)");
					}
				}
				...
				.append("\t\t\t\t\t\t\t<label class=\"control-label\" for=\"").append(columnName).append("\">").append(columnDescription).append("</label>\n")
				.append("\t\t\t\t\t\t\t<div class=\"controls\">\n");
				
				if("text".equalsIgnoreCase(columnType) || "number".equalsIgnoreCase(columnType)){
					formFields.append("\t\t\t\t\t\t\t\t<input id=\"").append(columnName).append("\" name=\"").append(columnName).append("\" type=\"text\">\n");
				} else if("textarea".equalsIgnoreCase(columnType)){
					formFields.append("\t\t\t\t\t\t\t\t<textarea id=\"").append(columnName).append("\" name=\"").append(columnName).append("\" rows=3></textarea>\n");
				} else if("date".equalsIgnoreCase(columnType)){
					formFields.append("\t\t\t\t\t\t\t\t<input type=\"datetime\" placeholder=\"请选择\" class=\"input-small\" autocomplete=\"off\" id=\"").append(columnName).append("\" name=\"").append(columnName).append("\">\n");
				}
		...
		if(queryPlaceHolder.toString().length() > 1){
			dataMap.put("queryPlaceHolder", new StringBuffer("输入").append(queryPlaceHolder.toString().substring(0, queryPlaceHolder.toString().length()-1)).append("...").toString());
		}
